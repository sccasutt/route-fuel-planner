...
export function RouteMarkers({
  ...
  useEffect(() => {
    ...
    // Ensure map is fully initialized with container
    if (!map.getContainer()) {
      console.log("Map container not available yet");
      return;
    }

    // Make sure the map's container is properly rendered in the DOM
    const container = map.getContainer();
    if (!container || !document.body.contains(container) || 
        container.clientHeight === 0 || container.clientWidth === 0) {
      console.log("Map container not properly rendered in DOM yet");
      return;
    }
    ...
    }, 300); // Delay marker addition to ensure map is ready
